...
class EGNN_vel_hidden(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,
        ...
        #self.add_module("gcl_0", E_GCL(in_node_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, act_fn=act_fn, recurrent=False, coords_weight=coords_weight))
        self.embedding = nn.Linear(in_node_nf, self.hidden_node_nf)
        self.add_module("gcl_%d" % 0, E_GCL_vel_hidden(self.hidden_node_nf, self.hidden_node_nf, self.hidden_edge_nf, self.hidden_node_nf, self.hidden_coord_nf, edges_in_d=in_edge_nf, act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent, norm_diff=norm_diff, tanh=tanh, num_vectors_out=num_vectors, color_steps=color_steps, ef_dim=ef_dim))
        for i in range(1, n_layers - 1):
            self.add_module("gcl_%d" % i, E_GCL_vel_hidden(self.hidden_node_nf, self.hidden_node_nf, self.hidden_edge_nf, self.hidden_node_nf, self.hidden_coord_nf, edges_in_d=in_edge_nf, act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent, norm_diff=norm_diff, tanh=tanh, num_vectors_in=num_vectors, num_vectors_out=num_vectors, color_steps=color_steps, ef_dim=ef_dim))
        self.add_module("gcl_%d" % (n_layers - 1), E_GCL_vel_hidden(self.hidden_node_nf, self.hidden_node_nf,self.hidden_edge_nf, self.hidden_node_nf, self.hidden_coord_nf, edges_in_d=in_edge_nf, act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent, norm_diff=norm_diff, tanh=tanh, num_vectors_in=num_vectors, last_layer=True, color_steps=color_steps, ef_dim=ef_dim))
        self.to(self.device)
